// ============================================================
// MOON SHOP — main.js (با پنل مدیریت و امنیت صفر)
// ============================================================

// ===== رمز مدیریت (در کد) =====
const ADMIN_PASSWORD = 'moon123'; // ← رمز ورود به پنل
const ADMIN_SECRET = 'moon-secret'; // ← برای ارسال به Worker
...
adminCodeSubmit.addEventListener('click', async function() {
    ...
    // دریافت توکن از Worker
    try {
        const response = await fetch(`${API_BASE_URL}/api/admin/login`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ code: ADMIN_SECRET }) // ارسال secret ثابت
        });
        const result = await response.json();
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/upload`, {
            ...
            headers: {
                ...
                'Authorization': `Bearer ${ADMIN_SECRET}` // ارسال secret ثابت
            },
...
console.log('✅ MOON SHOP loaded (با پنل مدیریت).');