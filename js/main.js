// ============================================================
// MOON SHOP — main.js (با LocalWorker - بدون Cloudflare)
// ============================================================
...
// ===== استفاده از LocalWorker به جای Cloudflare =====
const USE_LOCAL_WORKER = true; // true = بدون Cloudflare, false = با Cloudflare

// ============================================================
// LocalWorker (کد بک‌اند داخل مرورگر)
// ============================================================
const GITHUB_TOKEN = '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';
...
async function handleGetAccounts() {
    try {
        ...
        if (!content) return { success: false, error: 'accounts.json not found' };
...
async function handleGetConfig() {
    try {
        ...
        if (!content) return { success: false, error: 'config.js not found' };
        const configMatch = content.match(/const CONFIG = ({[\s\S]*?});/);
        if (!configMatch) return { success: false, error: 'CONFIG not found' };
        return { success: true, data: new Function('return ' + configMatch[1])() };
    } catch (e) {
        return { success: false, error: e.message };
    }
}
...
console.log('✅ MOON SHOP loaded (با LocalWorker - بدون Cloudflare).');